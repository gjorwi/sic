			...
			<legend class="subtitform linform" align="left"><i class="fa fa-search iconform" ></i>Busqueda</legend>
					...
					<div id="no-more-tables" >
			            <table class="ajuste table-bordered table-striped table-condensed cf" >
			        		...
			        		<tbody>
			        			...
			        			?>
			        			<tr>
			        				<td data-title="Codigo"><?=$row->id?></td>
			        				<td data-title="Nombre"><?=$row->nom_sis?></td>
			        				<td data-title="Estatus"><?=$estatus?></td>
			        				<td data-title="Accion" class="" align="center"><button class="btn btn-success yo" id="mod" onclick="location.href='<?=base_url();?>configuracion/sistemas/modificar/<?=$row->id?>'" style="font-size:11px;box-shadow:-1px 2px 1px -1px #226B12;border:none;">Editar</button></td>
			        			</tr>
			        			<?php
			        				}
			        			?>
...
<script type="text/javascript">
  ...
  function guardar(){
  	window.location.href='<?=base_url()?>configuracion/sistemas';
  }
  <?php
  if (isset($mensajes)) {
  ?>
