			            ...
			            <table class="ajuste table-bordered table-striped table-condensed cf" >
			        		...
			        		<tbody>
			        			...
			        			<tr>
			        				...
			        				<td data-title="Subsistema"><?=$row->nom_sub?></td>
			        				<td data-title="Sistema"><?=$row->sistema_nom_sis?></td>
			        				<td data-title="Estatus"><?=$estatus?></td>
			        				<td data-title="Accion" class="" align="center"><button class="btn btn-success yo" id="mod" onclick="location.href='<?=base_url();?>configuracion/subsistemas/modificar/<?=$row->id?>'" style="font-size:11px;box-shadow:-1px 2px 1px -1px #226B12;border:none;">Editar</button></td>
			        			</tr>
			        			<?php
			        				}
			        			?>
...
<script type="text/javascript">
  ...
  function guardar(){
  	window.location.href='<?=base_url()?>configuracion/subsistemas';
  }
  <?php
  if (isset($mensajes)) {
  ?>
