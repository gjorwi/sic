				...
				<div class="row ajuste">
					<div id="no-more-tables" >
			            <table class="ajuste table-bordered table-striped table-condensed cf" >
			        		...
			        		<tbody>
			        			<?php
			        				//print_r($reunions->objetivo);
			        				foreach ($reunions as $row) {
			        			?>
					        			<tr>

					        				<td data-title="M.T.A"><?=$row->mesatecnica_nom_mta?></td>
					        				<td data-title="Objetivo" style="max-width:200px;"><?=$row->objetivo?></td>
					        				<td data-title="Lugar" style="max-width:200px;"><?=$row->lugar?></td>
					        				<td data-title="Fecha" class=""><?=formatoFecha($row->fecha)?></td>
					        				<td data-title="Hora" class=""><?=$row->hora?></td>
					        				<td data-title="Accion" class="" align="center"><button class="btn btn-success yo" id="mod" onclick="location.href='<?=base_url();?>mta/reuniones/buscar/<?=$row->id?>'" style="font-size:11px;box-shadow:-1px 2px 1px -1px #226B12;border:none;">Ver</button></td>
					        			</tr>
			        			<?php
			        				}
			        			?>
...
<script type="text/javascript">
  ...
  function guardar(){
  	window.location.href='<?=base_url()?>mta/reuniones';
  }
  <?php
  if (isset($mensajes)) {
  ?>
