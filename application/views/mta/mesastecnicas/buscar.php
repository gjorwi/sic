					...
					<div id="no-more-tables" >
			            <table class="ajuste table-bordered table-striped table-condensed cf" >
			        		...
			        		<tbody>
			        			...
			        			?>
			        			<tr>
			        				<td data-title="RIF"><?=$row->tiporifmta?>-<?=$row->rifmta?></td>
			        				<td data-title="Nombre"><?=$row->nom_mta?></td>
			        				<td data-title="Fecha Conformacion"><?=validarEntrada($row->municipio_nom_mun)?></td>
			        				<td data-title="C. Comunal" class=""><?=$row->consejocomunal_nombrecc?></td>
			        				<td data-title="Estatus" class=""><?=validarEntrada($estatus)?></td>
			        				<td data-title="Accion" class="" align="center"><button class="btn btn-success yo" id="mod" onclick="location.href='<?=base_url();?>mta/mesastecnicas/modificar/<?=$row->id?>'" style="font-size:11px;box-shadow:-1px 2px 1px -1px #226B12;border:none;">Editar</button></td>
			        			</tr>
			        			<?php
			        				}
			        			?>
...
<script type="text/javascript">
	...
	function cerrar(){
		window.location.href='<?=base_url()?>mta/inicio';
	}
  ...
  function guardar(){
  	window.location.href='<?=base_url()?>mta/mesastecnicas';
  }
  <?php
  if (isset($mensajes)) {
  ?>
