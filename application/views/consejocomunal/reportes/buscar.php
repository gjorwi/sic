			...
			<legend style="border-color:#05B1A0;" align="left"><i class="fa fa-search" style="margin-right:5px;color:#E73C4B;"></i>Busqueda</legend>
					...
					<div id="no-more-tables" >
			            <table class="ajuste table-bordered table-striped table-condensed cf" >
			        		...
			        		<tbody>
			        			...
			        			?>
			        			<tr>
			        				<td data-title="Rif"><?=$row->rif?></td>
			        				<td data-title="Razon"><?=$row->razon?></td>
			        				<td data-title="Telefono" class=""><?=$row->telefonoh?></td>
			        				<td data-title="Estatus" class=""><?=$estatus?></td>
			        				<td data-title="Accion" class="" align="center"><button class="btn btn-success yo" id="mod" onclick="location.href='<?=base_url();?>configuracion/empresas/modificar/<?=$row->id?>'" style="font-size:11px;box-shadow:-1px 2px 1px -1px #226B12;border:none;">Editar</button></td>
			        			</tr>
			        			<?php
			        				}
			        			?>
...
<script type="text/javascript">
  ...
  function guardar(){
  	window.location.href='<?=base_url()?>configuracion/empresa';
  }
  <?php
  if (isset($mensajes)) {
  ?>
