				...
				<div class="row ajuste">
					<div id="no-more-tables" >
			            <table class="ajuste table-bordered table-striped table-condensed cf" >
			        		...
			        		<tbody>
			        			<?php
			        				foreach ($consejocomunales as $row) {
			        				$estatus="";
			        				if ($row->est_cco=="t") {
			        					$estatus="Activo";
			        				}
			        			...
			        			?>
			        			<tr>
			        				<td data-title="Rif"><?=$row->tiporifcc?>-<?=$row->rifcc?></td>
			        				<td data-title="Nombre"><?=$row->nombrecc?></td>
			        				<td data-title="Municipio" class=""><?=validarEntrada($row->municipio_nom_mun)?></td>
			        				<td data-title="Estatus" class=""><?=validarEntrada($estatus)?></td>
			        				<td data-title="Accion" class="" align="center"><button class="btn btn-success yo" id="mod" onclick="location.href='<?=base_url();?>consejocomunal/consejoscomunales/modificar/<?=$row->id?>'" style="font-size:11px;box-shadow:-1px 2px 1px -1px #226B12;border:none;">Editar</button></td>
			        			</tr>
			        			<?php
			        				}
			        			?>
...
<script type="text/javascript">
	...
	function cerrar(){
		window.location.href='<?=base_url()?>consejocomunal/inicio';
	}
  ...
  function guardar(){
  	window.location.href='<?=base_url()?>consejocomunal/consejoscomunales';
  }
  <?php
  if (isset($mensajes)) {
  ?>
